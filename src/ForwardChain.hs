...
-- Tries to add a consequent to the knowledge base as a new working assertion
-- if it is not already in the working assertions, it is added and the changed
-- flag is set to indicate that the working assertion set has changed.
-- Otherwise it returns the knowledge base as-is
...
-- Adds a list of rule consequents to the knowledge base
addConsequents :: [Consequent] -> KnowledgeBase -> VarMap -> KnowledgeBase
addConsequents cs kb vm =
  foldl' (addConsequent vm) kb cs

-- For each rule in the knowledge base, update the working assertions by trying
-- each rule to see if they generate any new information
...
-- Keep trying to fire rules until none of them produce any new working assertions    
...
-- Updates the knowledge base by copying the initial assertions into the working
-- assertions and then trying to fire rules until there are no new working assertions
-- generated by a rule
updateKnowledgeBase (KnowledgeBase a c _ _) =
  updateKnowledgeBase' (KnowledgeBase a c a False)

-- Since the working assertions contain all possible conclusions, just search through
-- them to see which ones match the query
